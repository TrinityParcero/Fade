            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                else if (count == 2)
                {
                    ...
                    for (int i = 0; i < line.Length; i++)
                    {
                        if (line[i] == 'g')
                        {
                            enemyList.Add(new Grunt(grunt, new Rectangle((playerLoc.X)+(Distance*(i+1)), (playerLoc.Y+42), 100, 100), 1, 1, 0.5));
                        }
                        else if (line[i] == 't')
                        {
                            enemyList.Add(new Tank(tank, new Rectangle(playerLoc.X+(Distance/ (i+1)), playerLoc.Y+20, 100, 100),1,3,1));
                        }
                    }//end of for loop
                }
                count++;
