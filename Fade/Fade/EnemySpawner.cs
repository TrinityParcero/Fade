            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                else if (count == 2)
                {
                    ...
                    for (int i = 0; i < line.Length; i++)
                    {
                        if (line[i] == 'g')
                        {
                            enemyList.Add(new Grunt(grunt, new Rectangle((playerLoc.X)+(Distance*(i+1)), (playerLoc.Y+40), 100, 100),1,1,1));
                        }
                        else if (line[i] == 't')
                        {
                            enemyList.Add(new Tank(tank, new Rectangle(playerLoc.X+(Distance/ (i+1)), playerLoc.Y+40, 100, 100),2,2,2));
                        }
                    }//end of for loop
                }
                count++;
